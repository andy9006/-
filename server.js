...
const WORD_BANKS = {
  ...
  relationship: {
    label: "戀愛夜生活",
    words: [
      "告白被拒", "交友軟體右滑", "看對方限動", "封鎖", "劈腿", "抓包", "吃醋", "冷戰",
      "分手復合", "深夜訊息", "酒後告白", "喝到斷片", "宿醉", "續攤", "拚酒", "夜店尬舞",
      "調酒師搖雪克杯", "開紅酒", "乾杯", "叫代駕", "同居", "求婚", "閃婚", "相親",
      "曖昧", "放閃", "偷看對方手機"
    ],
  },
};
const DEFAULT_BANK = "classic";
...
io.on("connection", (socket) => {
  ...
  socket.on("startGame", () => {
    ...
    if (room.roundActive || room.gameOver || room.roundNumber !== 0) return;
    if (room.order.length < 2) {
      socket.emit("errorMsg", "至少需要 2 人才能開始遊戲。");
      return;
    }
