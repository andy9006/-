...
2. 把這次的 `server.js`、`public/index.html`、`README.md` 重新上傳覆蓋原本的檔案（GitHub 網頁版：點檔案 → 鉛筆圖示編輯，或直接刪除舊檔重新上傳新檔）
3. Render 偵測到 GitHub 有更新後，會**自動重新部署**，約 1-2 分鐘後就能用新版本
4. 不需要重新設定 Web Service，網址也不會改變

## 🆕 這個版本新增的功能
- **建立房間時可自訂**：房間名稱、只有房主畫圖 / 輪流畫圖、隨機題庫（1-20 題）或自訂詞彙、作畫時間（10 / 30 / 60 秒）、上傳背景圖片
- **遊戲全部題目結束後**會彈出「最終排名結算」畫面，列出所有參與者的暱稱與分數，房主可以按「重新開始」用同樣設定再玩一輪
